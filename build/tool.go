...
func GenerateBatFile(nodenum, shardnum, modID int, isUTXO bool) {
	fileName := fmt.Sprintf("bat_shardNum=%v_NodeNum=%v_mod=%v_%v.bat", shardnum, nodenum, params.CommitteeMethod[modID], isUTXO)
	...
	for i := 1; i < nodenum; i++ {
		for j := 0; j < shardnum; j++ {
			if isUTXO {
				str = fmt.Sprintf("start cmd /k go run main.go -n %d -N %d -s %d -S %d -m %d -u\n\n", i, nodenum, j, shardnum, modID)
			} else {
	...
	if isUTXO {
		str = fmt.Sprintf("start cmd /k go run main.go -c -N %d -S %d -m %d -u\n\n", nodenum, shardnum, modID)
	} else {
		str = fmt.Sprintf("start cmd /k go run main.go -c -N %d -S %d -m %d \n\n", nodenum, shardnum, modID)
	}

	ofile.WriteString(str)
	// Need to run main node at the last time
	for j := 0; j < shardnum; j++ {
		if isUTXO {
			str = fmt.Sprintf("start cmd /k go run main.go -n 0 -N %d -s %d -S %d -m %d -u\n\n", nodenum, j, shardnum, modID)
		} else {
